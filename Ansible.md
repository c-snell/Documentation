...
First step to using the storage module is to download it from Github. As of Ansible 2.5 and earlier, you will need to download it from Github, after the release of Ansible 2.6, it will be part of Ansible core and included when you download Ansible.
...
It looks a bit different than the original ones we wrote. The playbooks provided in the module are actually **_templates_**. Note, that it is missing the `---` at the beginning of the **yaml** file, also if you look at the task names, you will see that there many different volume tasks (**_create_**, **_set snap cpg_**, **_grow volume_**, **_rename_**, and **_delete_**). Typically, you would not run all of these tasks within the same playbook but a subset of volume task plus other tasks maybe from the hosts playbook, or vlun management in order to create a playbook that automates the entire storage provisioning process.

If you compare these actions to what we wrote with the `uri` module and REST in the beginning, the storage module reduces the lines of code from 57 down to ~10. That is a significant simplification of the process, plus the introduction of HPE 3PAR specific resources, makes it much easier to identify what information we need to provide in order to run the playbook.

There are a couple of important concepts that I want to call out here to help you in your journey to learn Ansible and how to use these modules effectively.
